...
Geen druk om dagelijks te oefenen. Train wanneer je tijd hebt.`,
    ...
    faq: [
      ...
      {
        q: 'Wat doet "Boeken die aandacht nodig hebben"?',
        a: 'Onderaan in de scope-keuze van Doos Modus vind je deze speciale scope. Hij selecteert automatisch boeken die aandacht behoeven op basis van drie signalen: (1) boeken die volgens FSRS aan herhaling toe zijn, (2) boeken waar je gemiddeld trager mee bent dan met je andere boeken (zelfs als je binnen de tijd antwoordt — denk aan boeken die je in 8 seconden vindt terwijl je gemiddeld 3 seconden doet), en (3) boeken die je recent hebt gemist of waar de tijd opraakte. Eén van die drie signalen is genoeg om in de scope te komen. Net als de rest van Doos Modus heeft deze sessie geen invloed op je FSRS-schema of je voortgang in Quiz Modus. Antwoorden tijdens Doos Modus worden niet meegerekend voor de selectie — alleen Quiz Modus-antwoorden bouwen de data op. Deze scope is niet te combineren met andere groepen (zoals Pentateuch) — hij staat op zichzelf. Persoonlijke records worden niet bijgehouden voor deze scope, omdat de set boeken elke keer kan verschillen. De knop is mogelijk gedisableerd als je nog te weinig training-data hebt (minder dan 20 boeken in je geschiedenis) of als geen enkel boek aan een van de drie criteria voldoet — in dat laatste geval staat alles er goed bij.',
      },
      {
...
No pressure to practice daily. Train when you have the time.`,
    ...
    faq: [
      ...
      {
        q: 'What does "Books that need attention" do?',
        a: 'At the bottom of the scope picker in Box Mode you\'ll find this special scope. It automatically selects books that need attention based on three signals: (1) books FSRS thinks are due for review, (2) books you\'re consistently slower on than your other books (even when you answer within target time — think of a book you find in 8 seconds when your average is 3), and (3) books you recently missed or where time ran out. Any one of those three signals is enough to land in the scope. Just like the rest of Box Mode, this session has no effect on your FSRS schedule or your Quiz Mode progress. Answers given in Box Mode are not counted toward the selection — only Quiz Mode answers build up the data. This scope cannot be combined with other groups (like Pentateuch) — it stands on its own. Personal bests are not tracked for this scope, because the set of books may differ each time. The button may be disabled if you don\'t have enough training data yet (fewer than 20 books in your history) or if no book matches any of the three criteria — in the latter case, you\'re in good shape.',
      },
      {
